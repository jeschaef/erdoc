...
Entity attributes can also be multivalued where the attribute does not have a single value but an indefinite amount of values (like a set or list of values). 
The syntax to indicate a multivalued attribute is by appending an `@` symbol to the attribute definition like shown in the following example:
```erdoc
entity Book {
  inv_nr key
  author @
